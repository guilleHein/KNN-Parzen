...
Predict_test=predict_parzen(X_train,Labels_train,N_classes,h(i),X_test);
Predict_train=predict_parzen(X_train,Labels_train,N_classes,h(i),X_train);
parzen_Pe_train=sum(Labels_train ~=Predict_train)/length(Labels_train);
parzen_Pe_test=sum(Labels_test ~= Predict_test)/length(Labels_test);
h(i)
